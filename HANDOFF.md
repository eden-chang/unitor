...
- **2026-05-21** — **Stage 2a / Frontend — Groups + MyGroup on live backend.** Discovery's Groups tab and the MyGroup workspace now run against `GET/POST/PATCH /api/v1/groups`, `POST /api/v1/groups/{id}/apply`, `GET /api/v1/groups/{id}/applications`, `POST /api/v1/applications/{id}/{accept,decline}`, `POST /api/v1/groups/{id}/leave`, and `POST /api/v1/groups/{id}/confirm`.
  - **New API + hooks layer:** `api/groups.ts` wrappers for every group endpoint. `hooks/useGroups.ts` exposes `useGroupsList` / `useGroup` / `useMyGroup` (derives membership from the list) / `useGroupApplications` plus mutations for create / update / apply / accept / decline / leave / confirm. Cache invalidation is co-located with each mutation — accept fans out into group detail, applications, list, and the candidate's other "my applications" caches.
  - **`GroupsView`** drops the `FORMING_GROUPS` mock and reads `useGroupsList` (driven by section + recruiting filters from DiscoveryPage). The "Stage 2 preview" banner over the Groups tab is gone.
  - **`GroupCard`** takes a real `GroupListItem` shape. The mock "average overlap" and "needed skills" affordances are dropped — the server doesn't expose those derivations yet. Card now shows leader-named group title, member count + recruiting state, description preview, member avatars, application-question count, and the applied-status chip.
  - **`GroupDetailPanel`** rewritten on `useGroup` + `useApplyToGroup`. Inline error copy for `GROUP_NOT_FOUND` / `GROUP_NOT_RECRUITING` / `GROUP_ALREADY_CONFIRMED` / `ALREADY_IN_GROUP` / `DUPLICATE_APPLICATION` / `INVALID_QUESTION`. Apply button disables when the group isn't recruiting.
  - **`components/groups/MyGroup.tsx` (new)** extracts the prototype's ~280-line `MyGroup` from App.tsx (now deleted) and rewires it. Three states: empty (no group → "Create a Group" form), member (group info + leave), leader (member view + edit name/description/recruiting + replace-set application questions + applications inbox with accept/decline + initiate confirmation).
  - **Leader-edit card** is wrapped with `key={group.id}` so the local form re-seeds via initial state on group switch — avoids the `react-hooks/set-state-in-effect` anti-pattern. After save, the draft re-seeds from the mutation's response so any server-side normalization shows up.
  - **App.tsx** lost ~280 lines (`function MyGroup` + `MyGroupProps` interface + `ConfirmStage` type + unused `Fragment` / `Checkbox` / `ConfirmDialog` imports). Page-map entry now points at `LiveMyGroup`. Lint count unchanged from baseline (still 1 error, 12 warnings — the ChatsPage one).
  - Backend untouched (already shipped in commit `1594ad8`). Frontend `npm run typecheck` + `npm run build` clean.
  - **Deferred:** per-member confirmation endpoint (needed for `confirming` → `confirmed`); notifications producers wiring (phase 2c); application-vote endpoint.
- **2026-05-21** — **Stage 2a / Backend — group lifecycle endpoints.** ORM for the migration-0005 tables, schemas, service, and routes for the full group lifecycle (create / get / update / apply / list-applications / accept / decline / leave / confirm). Writes flow through `admin_session` per migration 0004's design; `app/db/admin.py`'s restricted-import comment extended to allow `app/api/v1/groups.py`. Accepting an application auto-withdraws the applicant's other pending applications for the same course. Tests 71 → 90. Detail: [`.docs/session-logs/2026-05-21-stage2a-backend.md`](./.docs/session-logs/2026-05-21-stage2a-backend.md). Commit `1594ad8`.
...
- **2026-05-20** — **Stage 1 / Step D — Profile wizard + edit page wired to backend.** The 4-step onboarding wizard and ProfileEdit screen no longer use mock data.
  ...
  - **Wizard:**
    - **Step 0** pre-fills from `auth.user.display_name`. Next calls `apiAuth.updateMe(name)` then `refreshBootstrap()` so Nav picks it up. Photo upload disabled with a "stage 2" hint.
    - **Step 1** loads the real catalog via `useCourseSkills`. Selection persists into wizard state. Custom-skill button removed (no backend support yet).
    - **Step 2** reads `section_code` from the first enrollment (TA-assigned via roster). Schedule cells / flexible flag persisted to wizard state. Next requires at least one cell or `flexible = true`.
    - **Step 3** submits everything in one call: `POST /profiles` with `{ skills, schedule_slots, ... }`. On `PROFILE_ALREADY_EXISTS` (partial wizard run), falls back to `PATCH /profiles/{id}` + `PUT /skills` + `PUT /schedule` for idempotency. Multi-platform selection collapsed to a single-platform pick (matches the backend's `comm_tool` shape). After success, `wizard.reset()` clears localStorage.
  - **ProfileDone** polls `POST /profiles/{id}/complete` on mount. If `is_complete=false`, surfaces the missing fields + an "Edit Profile" link inline; the "Go to Matching Board" CTA is disabled until complete.
  - **ProfileEdit** dropped `useLocalStorage` entirely. Reads via `useMyProfile`, mutates via the new hooks. Save runs `PATCH /profiles/{id}` then `PUT /skills` then `PUT /schedule` in sequence; Cancel discards the local draft (server snapshot is the source of truth). Empty/loading/error/no-profile states each get their own minimal shell. Skill chips render from the real catalog so the name shows even if the user later removes the skill from selection.
  - **Cleanup:** removed the unused-warning surface a bit by dropping `useEffect`-based draft seeding (Cancel/enter-edit are synchronous now).
  - All Step C frontend tests still green: backend 71 unit tests pass, frontend `npm run typecheck` + `npm run build` clean, lint unchanged (1 pre-existing error in the not-yet-extracted ChatsPage).
  - **Still on mock data:** Discovery board (next: Step E) and the out-of-scope pages (MyGroup / Chats / Urgent / TA*).
